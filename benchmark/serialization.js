...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new einrcore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new einrcore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = einrcore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new einrcore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function einrcoreTest() {
      block1 = einrcore.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('einrcore', einrcoreTest, {maxTime: maxTime});
