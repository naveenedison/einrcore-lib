...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of einrcore methods. For example, calling `JSON.stringify` on a einrcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
